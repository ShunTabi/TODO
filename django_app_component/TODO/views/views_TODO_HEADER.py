...
# 定義
sql_1 = "SELECT TODO_HEADER_ID, TODO_HEADER_NAME , GENRE_NAME, TODO_HEADER_DATE FROM V_TODO_HEADER WHERE GENRE_VISIBLESTATUS = ? AND TODO_HEADER_VISIBLESTATUS = ? ORDER BY TODO_HEADER_DATE LIMIT ? OFFSET ?"
sql_1_count = "SELECT COUNT(*) FROM V_TODO_HEADER WHERE GENRE_VISIBLESTATUS = ? AND TODO_HEADER_VISIBLESTATUS = ? ORDER BY TODO_HEADER_DATE"
sql_2 = "SELECT TODO_HEADER_ID, TODO_HEADER_NAME , GENRE_NAME, TODO_HEADER_DATE FROM V_TODO_HEADER WHERE TODO_HEADER_ID NOT IN (SELECT TODO_HEADER_ID FROM V_TODO_HEADER WHERE GENRE_VISIBLESTATUS = ? AND TODO_HEADER_VISIBLESTATUS = ?) ORDER BY TODO_HEADER_DATE LIMIT ? OFFSET ?"
sql_2_count = "SELECT COUNT(*) FROM V_TODO_HEADER WHERE TODO_HEADER_ID NOT IN (SELECT TODO_HEADER_ID FROM V_TODO_HEADER WHERE GENRE_VISIBLESTATUS = ? AND TODO_HEADER_VISIBLESTATUS = ?)"
...
sql_6_A = "INSERT INTO T_TODO_HEADER(TODO_HEADER_NAME,GENRE_ID,PRIOR_ID,TODO_HEADER_STARTDATE,TODO_HEADER_ENDDATE) VALUES(?,(SELECT GENRE_ID FROM T_GENRE WHERE GENRE_NAME = ?),(SELECT PRIOR_ID FROM T_PRIOR WHERE PRIOR_NAME = ?),?,?)"
...
def TODO_HEADER_TOP(req, TODO_PAGE):
    sql_params = (
        0, 0, sql_limit, sql_limit*(TODO_PAGE-1),
    )
    sql_params_count = (
        0, 0,
    )
