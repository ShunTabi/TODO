...
# 定義
sql_1 = "SELECT TODO_DETAIL_ID,GENRE_NAME,TODO_HEADER_NAME,TODO_DETAIL_NAME,PRIOR_SUBNAME,TODO_DETAIL_STARTDATE,TODO_DETAIL_ENDDATE,TODO_DETAIL_VISIBLESTATUS FROM V_TODO_DETAIL WHERE PRIOR_ID != (SELECT PRIOR_ID FROM T_PRIOR WHERE PRIOR_SUBNAME = ? ) AND TODO_DETAIL_VISIBLESTATUS = ? ORDER BY TODO_DETAIL_ENDDATE,PRIOR_ID LIMIT ? OFFSET ?"
sql_1_count = "SELECT COUNT(*) FROM V_TODO_DETAIL WHERE PRIOR_ID != (SELECT PRIOR_ID FROM T_PRIOR WHERE PRIOR_SUBNAME = ? ) AND TODO_DETAIL_VISIBLESTATUS = ?"
...
sql_7 = "UPDATE T_TODO_DETAIL SET TODO_HEADER_ID = (SELECT TODO_HEADER_ID FROM T_TODO_HEADER WHERE TODO_HEADER_NAME =?),PRIOR_ID = (SELECT PRIOR_ID FROM T_PRIOR WHERE PRIOR_NAME = ?),TODO_DETAIL_NAME = ?,TODO_DETAIL_STARTDATE = ?,TODO_DETAIL_ENDDATE = ?,TODO_DETAIL_VISIBLESTATUS = ? WHERE TODO_DETAIL_ID = ?"
...
def TODO_DETAIL_TOP(req, TODO_DETAIL_PAGE):
    sql_params = (
        '済',  0, sql_limit, sql_limit*(TODO_DETAIL_PAGE-1),
    )
    sql_params_count = (
        '済',  0,
    )
...
def TODO_DETAIL_TOP_DEL(req, TODO_DETAIL_PAGE):
    sql_params = (
        '済', 1, sql_limit, sql_limit*(TODO_DETAIL_PAGE-1),
    )
    sql_params_count = (
        '済',  1,
    )
