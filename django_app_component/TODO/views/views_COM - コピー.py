...
def values_GOAL2(GOAL_ID,TODO_HEADER_ID):
    if(GOAL_ID != 0):
        return GOAL_ID
    sql_4 = "SELECT GOAL_ID FROM V_TODO_HEADER WHERE TODO_HEADER_ID = ? AND TODO_HEADER_VISIBLESTATUS = ?"
    if(TODO_HEADER_ID == 0):
        return 
...
def values_TODO_HEADER():
    sql_5 = "SELECT TODO_HEADER_ID,TODO_HEADER_NAME FROM V_TODO_HEADER WHERE TODO_HEADER_VISIBLESTATUS = ?"
    sql_params = (
        0,
    )
...
def values_TODO_HEADER2(GOAL_ID):
    sql_6 = "SELECT TODO_HEADER_ID,TODO_HEADER_NAME FROM V_TODO_HEADER WHERE GOAL_ID = ? AND TODO_HEADER_VISIBLESTATUS = ?"
    if(GOAL_ID == 0):
...
def values_STATUS():
    sql_7 = "SELECT STATUS_ID,STATUS_NAME FROM T_STATUS WHERE STATUS_VISIBLESTATUS = ?"
    sql_params = (
        0,
    )
